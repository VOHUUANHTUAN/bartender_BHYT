...
const FinancialReport = () => {
  ...
  const [totalAmountOfRegistration, setTotalAmountOfRegistration] = useState(0);
    ...
    const fetchFinancialReport = async () => {
      try {
        ...
        const filteredData = financialReportData.filter((row) => {
          ...
          return (
            (selectedYear === 'Tất cả' || rowDate.getFullYear() === parseInt(selectedYear)) &&
            (selectedMonth === null || rowDate.getMonth() + 1 === selectedMonth)
          );
        });
  ...
  const columns = [
    { field: "stt", headerName: "STT", width: 100 },
    {
      ...
      renderCell: (params) => {
        ...
        const sign = params.row.loaiHoaDon === "Đơn đăng ký" ? "+" : "-";
        return (
        ...
        <div style={{ display: "flex", gap: "8px",marginTop: "10px", }}>
          ...
          >
            ...
            >
              ...
              <div style={{ fontSize: "24px" }}>
                {formatCurrency(
                  totalAmountOfRegistration - totalAmountOfRefund
                )}
              </div>
            </div>
          </div>
